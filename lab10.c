...
Sockets are made non-blocking from the line fcntl(fd, F_SETFL, flags |
O_NONBLOCK).

The server sockers for accept() and the client sockets for read() and made
non-blocking.

They are made non-blocking to prevent deadlocks and allow for a clean shutdown
if needed.
